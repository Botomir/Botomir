...
function setRoleMessageCommand(message, args) {
    let parts = parseRoleMessage(args.join(' '));

    // lookup the actual emoji to use react with
    let mappings = parts.mappings.map((m) => {
        const reactionEmoji = emojiRegex().exec(m.emoji) ? m.emoji : message.guild.emojis.cache.find(emoji => emoji.name === m.emoji);
        return {reactionEmoji, mapping: m};
    });
