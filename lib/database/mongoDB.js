...
const BotResponses = mongoose.model('botResponses', {
    guild: String,
    message: String,
    botMode: String,
});
...
function clearRoleMappings(serverID) {
    return RoleEmojis.deleteMany({
        guild: serverID,
    });
}
...
function addBotResponse(serverID, message, botMode) {
    const response = new BotResponses({
        guild: serverID,
        message,
        botMode,
    });
...
function loadBotResponses(serverID, botMode) {
    return BotResponses.find({
        guild: serverID, botMode,
    });
}
